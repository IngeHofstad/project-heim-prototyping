...
/*
 * Clone of CharacterAnimator, with support for second animator (for mirror).
 */
...
namespace TraversalPro
{
    ...
    public class CharacterAnimator_Modified : MonoBehaviour
    {
        ...
        void OnEnable()
        {
            ...
            if (!ValidateRequiredField(this, characterMotor.Value)
                | !ValidateRequiredField(this, jump.Value))
            {
                enabled = false;
            }
        }
        ...
        void LateUpdate()
        {
            if (groundedId.HasValue)
            {
                animator.SetBool(groundedId.Value, characterMotor.Value.IsGrounded);
                if (_secondAnimator != null)
                    _secondAnimator.SetBool(groundedId.Value, characterMotor.Value.IsGrounded);
            }
            ...
            if (freeFallDurationId.HasValue)
            {
                animator.SetFloat(freeFallDurationId.Value, currentFreeFallDuration);
                if (_secondAnimator != null)
                    _secondAnimator.SetFloat(freeFallDurationId.Value, currentFreeFallDuration);
            }
            
            Vector3 runVelocityGoal = characterMotor.Value.Rigidbody.GetVelocity() - characterMotor.Value.Ground.PointSecantVelocity;
            ...
            if (velocityXId.HasValue)
            {
                animator.SetFloat(velocityXId.Value, runValue.X);
                if (_secondAnimator != null)
                    _secondAnimator.SetFloat(velocityXId.Value, runValue.X);
            }
            if (velocityYId.HasValue)
            {
                animator.SetFloat(velocityYId.Value, runValue.Y);
                if (_secondAnimator != null)
                    _secondAnimator.SetFloat(velocityYId.Value, runValue.Y);
            }
            if (animationSpeedId.HasValue)
            {
                animator.SetFloat(animationSpeedId.Value, Mathf.Max(runValue.Magnitude, 1));
                if (_secondAnimator != null)
                    _secondAnimator.SetFloat(animationSpeedId.Value, Mathf.Max(runValue.Magnitude, 1));
            }
            
            bool hasRecentlyJumped = Time.timeAsDouble < jump.Value.LastJumpTime + recentJumpDuration;
            if (jumpId.HasValue)
            {
                animator.SetBool(jumpId.Value, hasRecentlyJumped);
                if (_secondAnimator != null)
                    _secondAnimator.SetBool(jumpId.Value, hasRecentlyJumped);
            }
